...
def bruteforce(): 
	...
	min_1 = 0				#min of list 1
	min_2 = 0				#min of list 2
	cur_min = 0				#initialize minimum

	minimum_points = []			#list of points that meet the distance requirement

	for line in cur_file:
		point = (int(line[0]), int(line[2]))
		point_list.append(point)
	
	sorted_point_list = sorted(point_list)
	
	#recursively find the points for the left side
	for i in range(0, (len(sorted_point_list)/2))
	...
	if len(point_list) >= 2:
		...
		sorted_point_list = sorted(point_list)
		median = len(sorted_point_list)/2	
		
		#recursively find the points for the left side
		for a in range(0, (median - 1))
			for b in range(a+1, median):
